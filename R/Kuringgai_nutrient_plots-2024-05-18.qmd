...
data_for_S2 <-
  vegetative_nutrient_invest_ind(SummaryInd_4a, leaf_var = "leaf_inv_gross", stem_var = "stem_inv_gross", resorb = FALSE) %>%
  filter(individual != "PUTU_108") %>%
  dplyr::bind_rows(repro_investment_spp_age) %>%
  calculate_proportions_spp_age() %>%
  mutate(
...
data_for_S3 <-
  vegetative_nutrient_invest(SummaryInd_S3, leaf_var = "leaf_inv_surplus", stem_var = "stem_inv_surplus", resorb = TRUE) %>%
  filter(individual != "PUTU_108") %>%
  dplyr::bind_rows(repro_investment_spp_age) %>%
  calculate_proportions_spp_age() %>%
  mutate(
