...
data <- SummarySpp[["mean"]] %>%
  ...
  mutate(name = factor(x=name, levels = c("prop_prepollen_discarded_vs_all_repro", "prop_postpollen_discarded_vs_all_repro",
                                          ...
                                          "prop_embryo_endo_vs_all_repro"))) %>%
  arrange(embryo_endo_costs) %>%
  select(-embryo_endo_costs) %>%
  dplyr::rename(accessory_category = name, proportion = value) %>%
  mutate(species_factor = factor(species, levels = c("EPMI", "PILI", "HEPU", "LEES", "COER", "PUTU", "PHPH", 
                                                     "BOLE", "PEPU", "PELA", "HATE", "GRSP", "BAER", "GRBU")))
...
Figure_2 <-
  ggplot(data = data, aes(x = species_factor, y = proportion)) +
...
Panel3B <-
  ...
  scale_color_manual(values = color_family) + 
    ...
    ggtitle("B. Seed mass - seedset relationship") +
    theme_info +
...
cowplot::save_plot("ms/Accessory/output/Figure3.pdf", Figure3, base_width = 6, base_height = 8.5)
...
mrpmm_cor(fit_MR1, "embryo_endo_costs", "prop_pollen_attract_vs_success", "phylo", "units", summary = T) fit \<- fit_MR1 y1 \<- "embryo_endo_costs" y2 \<- "prop_pollen_attract_vs_success"

### New code for creating tree, Nov 2024

```{r}
data <- species_full |> 
  select(-species, -Abbreviation) |> 
  rename(species = Species_name, family = Family) |> 
  mutate(genus = word(species, 1)) |> 
  select(species, genus, family) |> 
  as.data.frame()

phy_new <- V.PhyloMaker2::phylo.maker(data)

phylogeny_plot <- plot(phy_new$scenario.3)

ggsave("ms/Accessory/output/Phylogeny.pdf", plot = phylogeny_plot, width = 8, height = 3)
```
...
plot(phy, show.tip.label = T) # looks ultrametric

# use 2024 tree instead

phy <- phy_new$scenario.3
...
#### seed mass \~ prop success costs to attraction
...
#### seed mass \~ prop pollen-attract to success
...
#### seed mass \~ prop provisioning to success
...
ggsave("ms/Accessory/output/Figure4Nov.pdf", Figure4, width = 9, height= 4.5)
```
...
data_5Ax <- data_5A %>% mutate(
  ratio_off_seedmass = repro_inv/embryo_endo_inv
) %>%
  arrange(ratio_off_seedmass)
...
data_5Bx <- data_5B %>% mutate(
  ratio_off = repro_inv/flower_inv
) %>%
  arrange(ratio_off)
...
cowplot::save_plot("ms/Accessory/output/Figure5.pdf", Figure5, base_width = 8, base_height = 4)
```
...
acc_costs_spp_format_sorted <- 
  ...
  mutate(
    ...
    `Seed mass` = paste0(embryo_endo_costs, " (", format(prop_embryo_endo_vs_all_repro,  nsmall = 1), "%)"),
  ...
  select(species, spp, seedset, repro_costs, `Pollen attraction`, `Packaging and dispersal`, `Seed mass`, 
...
accessory_using_nutrients <- readr::read_csv("data/accessory_data_in_nutrient_currency.csv")
...
grid::grid.draw(Figure3N)

cowplot::save_plot("ms/Accessory/output/Figure3N.png", Figure3N, base_width = 6, base_height = 8.5)
...
Panel3B <-
  ggplot(data = data, aes(x = embryo_endo_costs, y = seedset)) +
    ...
    scale_y_log10(limits = c(0.001,1.2), labels = label_log(), breaks = c(.001, .01, .1, 1)) +
    geom_smooth(aes(x=embryo_endo_costs, y = 10^(fit3B$fitted.values)), colour="#CC1A36") + 
  scale_color_manual(values = color_family) + 
    guides(x.sec = "axis", y.sec = "axis") +
    labs(
...
cowplot::save_plot("ms/Accessory/output/Figure3P.png", Figure3P, base_width = 6, base_height = 8.5)
...
# remove prop_provisioning_vs_success, because 1:1 correlation with `prop_pollen_attract_vs_success`, since they sum to 1
for_PCA <- 
  data %>% 
  dplyr::select(-species, -prop_provisioning_vs_success, -Family) %>%
  vegan::decostand(method = "standardize") # since different scaling for different traits
